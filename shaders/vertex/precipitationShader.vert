...
#define initalMass 0.15                             // 0.05 initial droplet mass
      ...
      if (spawnChance > nrmRand) {                                       // spawn precipitation particle
        ...
        if (realTemp < CtoK(0.0)) {                                      // below 0 C
          ...
          if (hailEnabled > 0.5 && stormDensity > hailStormDensityThreshold &&
            ...
            newMass[ICE] = initalMass + min(capeExcess * 0.0002, 1.5); // same starting mass as snow, just grows heavier with CAPE
  ...
  if (isActive) {
    ...
    if (totalMass < 0.04) { // to small
      ...
      // Hail uses the exact same motion rule as snow/rain -- no special-cased branch. It simply falls
      // faster because it IS heavier: newDensity (up to 2.5 for hail, vs 0.1-0.9 for snow) and totalMass
      // both feed directly into this one shared formula, same as they always have.
      newPos += base.xy / resolution * 2.0;
